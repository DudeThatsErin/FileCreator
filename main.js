...
class FileCreatorPlugin extends Plugin {
    async onload() {
        ...
        // Add individual commands for each file type
        this.addCommand({
            id: 'create-markdown-file',
            name: 'Create Markdown File',
            callback: () => this.createFileWithDefaults('markdown')
        });

        this.addCommand({
            id: 'create-pdf-file',
            name: 'Create PDF File',
            callback: () => this.createFileWithDefaults('pdf')
        });

        this.addCommand({
            id: 'create-kanban-board',
            name: 'Create Kanban Board',
            callback: () => this.createFileWithDefaults('kanban')
        });

        this.addCommand({
            id: 'create-base-file',
            name: 'Create Base File',
            callback: () => this.createFileWithDefaults('base')
        });

        this.addCommand({
            id: 'create-excalidraw-drawing',
            name: 'Create Excalidraw Drawing',
            callback: () => this.createFileWithDefaults('excalidraw')
        });
    }
    ...
    async createFileWithDefaults(fileType) {
        ...
        const now = new Date();
        const timestamp = this.formatDate(now, this.settings.dateFormat);
        let fileName = `New ${fileType}`;
        
        if (this.settings.datePosition === 'prefix') {
            fileName = `${timestamp}-${fileName}`;
        } else if (this.settings.datePosition === 'suffix') {
        ...
        if (await this.app.vault.adapter.exists(filePath)) {
            new Notice(`File already exists: ${fileName}${ext}`);
            return;
        }
        ...
        try {
            ...
            if (fileType === 'kanban') {
                ...
                if (this.settings.defaultBaseTemplate && this.settings.defaultBaseTemplate !== 'none') {
                    const templatePath = `${this.settings.baseTemplatesPath}/${this.settings.defaultBaseTemplate}.base`;
                ...
                if (this.settings.defaultExcalidrawTemplate && this.settings.defaultExcalidrawTemplate !== 'none') {
                    const templatePath = `${this.settings.excalidrawTemplatesPath}/${this.settings.defaultExcalidrawTemplate}.excalidraw.md`;
                    const templateFile = this.app.vault.getAbstractFileByPath(templatePath);
                    if (templateFile) {
                        content = await this.app.vault.read(templateFile);
                    } else {
                        content = this.getDefaultExcalidrawContent();
                    }
                } else {
                    content = this.getDefaultExcalidrawContent();
                }
            } else if (fileType === 'markdown') {
                if (this.settings.defaultMdTemplate && this.settings.defaultMdTemplate !== 'none') {
                    const templatePath = `${this.settings.mdTemplatesPath}/${this.settings.defaultMdTemplate}`;
                    try {
                        content = await this.app.vault.adapter.read(templatePath);
                    } catch (error) {
                        // Template not found, use empty content
                    }
                }
            } else if (fileType === 'pdf') {
                if (this.settings.defaultPdfTemplate && this.settings.defaultPdfTemplate !== 'none') {
                    const templatePath = `${this.settings.pdfTemplatesPath}/${this.settings.defaultPdfTemplate}`;
                    try {
                        const data = await this.app.vault.adapter.readBinary(templatePath);
                        await this.app.vault.adapter.writeBinary(filePath, data);
                        new Notice(`Created PDF: ${fileName}${ext}`);
                        return;
                    } catch (error) {
                        new Notice('PDF template not found, creating empty file');
                    }
                }
            }
            ...
            new Notice(`Created: ${fileName}${ext}`);
    ...
    formatDate(date, format) {
        ...
        return format
            ...
            .replace('MM', month)
            .replace('dd', day)
            .replace('MMddYYYY', `${month}${day}${year}`);
    }

    generateKanbanContent() {
        const headers = this.settings.defaultKanbanHeaders.split(',').map(h => h.trim());
        const completedHeaders = this.settings.kanbanCompletedHeaders.split(',').map(h => h.trim());
        
        let content = '---\n\nkanban-plugin: basic\n\n---\n\n';
        
        headers.forEach(header => {
            const isCompleted = completedHeaders.includes(header);
            content += `## ${header}\n\n`;
            if (isCompleted) {
                content += `**Complete**\n\n`;
            }
